...
export async function updateLoyaltySettings(data: Record<string, unknown>) {
  ...
  if (existing) {
    await db.update(loyaltySettings).set({ ...data, updatedAt: new Date() } as any).where(eq(loyaltySettings.id, existing.id));
  } else {
    await db.insert(loyaltySettings).values({ ...data, createdAt: new Date(), updatedAt: new Date() } as any);
  }
  return true;
}
...
export async function createOrUpdateLoyaltyPoints(userId: number, currentBalance: number) {
  const db = await getDb();
  if (!db) return false;
  const existing = await getUserLoyaltyPoints(userId);
  if (existing) {
    await db.update(loyaltyPoints).set({ currentBalance, updatedAt: new Date() }).where(eq(loyaltyPoints.userId, userId));
  } else {
...
export async function addLoyaltyTransaction(userId: number, transactionType: string, points: number, reason: string, balanceBefore: number, balanceAfter: number, orderId?: number, expiryDate?: Date) {
  const db = await getDb();
  if (!db) return false;
...
export async function getLoyaltyTransactions(userId: number, limit = 50, offset = 0) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(loyaltyTransactions).where(eq(loyaltyTransactions.userId, userId)).orderBy(desc(loyaltyTransactions.createdAt)).limit(limit).offset(offset);
}
...
export async function updateReferralSettings(data: Record<string, unknown>) {
  const db = await getDb();
  if (!db) return false;
  const existing = await getReferralSettings();
  if (existing) {
    await db.update(referralSettings).set({ ...data, updatedAt: new Date() } as any).where(eq(referralSettings.id, existing.id));
  } else {
    await db.insert(referralSettings).values({ ...data, createdAt: new Date(), updatedAt: new Date() } as any);
  }
  return true;
}
...
export async function getUserReferralLink(userId: number) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(referralLinks).where(eq(referralLinks.referrerId, userId)).limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function createReferralLink(userId: number, referralCode: string, referralUrl: string) {
  const db = await getDb();
  if (!db) return null;
...
export async function getReferralLinkByCode(referralCode: string) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(referralLinks).where(eq(referralLinks.referralCode, referralCode)).limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function addReferralTransaction(referrerId: number, referredUserId: number, referralLinkId: number, rewardAmount: number, rewardType: string, orderId?: number, notes?: string) {
  const db = await getDb();
  if (!db) return false;
...
export async function getReferralTransactions(referrerId: number, limit = 50, offset = 0) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(referralTransactions).where(eq(referralTransactions.referrerId, referrerId)).orderBy(desc(referralTransactions.createdAt)).limit(limit).offset(offset);
}

export async function updateReferralTransaction(transactionId: number, status: string) {
  const db = await getDb();
  if (!db) return false;
