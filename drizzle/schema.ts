...
/**
 * Loyalty Points Settings - Admin configuration for loyalty program
 */
export const loyaltySettings = mysqlTable("loyalty_settings", {
  id: int("id").autoincrement().primaryKey(),
  pointsPerRupee: decimal("pointsPerRupee", { precision: 5, scale: 2 }).default("1").notNull(), // 1 rupee = X points
  redemptionRate: decimal("redemptionRate", { precision: 12, scale: 2 }).default("1").notNull(), // X points = ₹1
  pointsExpiryDays: int("pointsExpiryDays").default(365), // Days before points expire (null = never)
  minPointsToRedeem: int("minPointsToRedeem").default(100), // Minimum points needed to redeem
  maxPointsPerOrder: int("maxPointsPerOrder"), // Max points that can be earned per order
...
/**
 * Customer Loyalty Points - Track loyalty points balance for each customer
 */
export const loyaltyPoints = mysqlTable("loyalty_points", {
  ...
  userIdIdx: index("loyalty_points_user_idx").on(table.userId),
}));
...
/**
 * Loyalty Points Transactions - Track all points earn/redeem transactions
 */
export const loyaltyTransactions = mysqlTable("loyalty_transactions", {
  ...
  expiryDate: timestamp("expiryDate"), // When these points expire
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  userIdIdx: index("loyalty_transactions_user_idx").on(table.userId),
  orderIdIdx: index("loyalty_transactions_order_idx").on(table.orderId),
  createdAtIdx: index("loyalty_transactions_created_idx").on(table.createdAt),
}));
...
/**
 * Referral Program Settings - Admin configuration for referral rewards
 */
export const referralSettings = mysqlTable("referral_settings", {
  id: int("id").autoincrement().primaryKey(),
  referrerRewardAmount: decimal("referrerRewardAmount", { precision: 12, scale: 2 }).notNull(), // Amount referrer gets
  referrerRewardType: mysqlEnum("referrerRewardType", ["cash", "points", "discount"]).default("cash").notNull(),
  referredRewardAmount: decimal("referredRewardAmount", { precision: 12, scale: 2 }).notNull(), // Amount referred customer gets
  referredRewardType: mysqlEnum("referredRewardType", ["cash", "points", "discount"]).default("discount").notNull(),
  minOrderValueForReward: decimal("minOrderValueForReward", { precision: 12, scale: 2 }).default("0"), // Min order value to trigger reward
  maxRewardsPerReferrer: int("maxRewardsPerReferrer"), // Max referrals that can earn reward per user
  referralLinkExpiryDays: int("referralLinkExpiryDays").default(90), // Days before referral link expires
  isEnabled: boolean("isEnabled").default(true),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
/**
 * Referral Links - Track unique referral codes for each user
 */
export const referralLinks = mysqlTable("referral_links", {
  ...
  totalConverted: int("totalConverted").default(0), // Referrals that made a purchase
  totalEarnings: decimal("totalEarnings", { precision: 12, scale: 2 }).default("0"),
  isActive: boolean("isActive").default(true),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  referrerIdIdx: index("referral_links_referrer_idx").on(table.referrerId),
  referralCodeIdx: index("referral_links_code_idx").on(table.referralCode),
}));
...
/**
 * Referral Transactions - Track referral rewards earned
 */
export const referralTransactions = mysqlTable("referral_transactions", {
  ...
  referralLinkId: int("referralLinkId").notNull().references(() => referralLinks.id, { onDelete: "cascade" }),
  orderId: int("orderId").references(() => orders.id, { onDelete: "set null" }),
  rewardAmount: decimal("rewardAmount", { precision: 12, scale: 2 }).notNull(),
  rewardType: mysqlEnum("rewardType", ["cash", "points", "discount"]).notNull(),
  status: mysqlEnum("status", ["pending", "completed", "cancelled"]).default("pending").notNull(),
  notes: text("notes"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  referrerIdIdx: index("referral_transactions_referrer_idx").on(table.referrerId),
  referredUserIdIdx: index("referral_transactions_referred_idx").on(table.referredUserId),
  orderIdIdx: index("referral_transactions_order_idx").on(table.orderId),
  statusIdx: index("referral_transactions_status_idx").on(table.status),
}));
